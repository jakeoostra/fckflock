...
export default function Home() {
  return (
    <>
      {/* ============== HERO ============== */}
      <section className="relative min-h-[88vh] px-4 sm:px-8 pt-20 pb-24 overflow-hidden scanlines">
        ...
        />
        {/* Big red corner brackets */}
        <Bracket className="top-20 left-4 sm:left-8" />
        <Bracket className="top-20 right-4 sm:right-8" rotate={90} />
        <Bracket className="bottom-24 left-4 sm:left-8" rotate={-90} />
        <Bracket className="bottom-24 right-4 sm:right-8" rotate={180} />

        <div className="relative grid lg:grid-cols-12 gap-10 max-w-[1500px] mx-auto items-center">
          {/* Title block */}
          ...
          <div className="lg:col-span-5 relative hidden lg:block">
            <div className="relative">
              <CameraSVG className="w-full max-w-[440px] mx-auto opacity-95" />
              ...
              {/* Target-acquired CTA — overlays the camera, vertically centered
                  between the "Subject 01" tag and the "Flock Falcon-class" caption. */}
              <Link
                to="/act"
                aria-label="Take down Madison, SD — join the campaign"
                className="group absolute top-1/2 left-2 -translate-x-1/2 -translate-y-1/2 z-30 flex flex-col items-center gap-2.5"
              >
                {/* Targeting indicator above the button */}
                <div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.3em] text-[var(--color-blood)]">
                  <span className="w-1.5 h-1.5 rounded-full bg-[var(--color-blood)] pulse-dot" />
                  <span>Target Acquired</span>
                </div>

                {/* Button body with corner brackets */}
                <div className="relative">
                  {/* Corner brackets */}
                  <span aria-hidden className="absolute -top-1.5 -left-1.5 w-3 h-3 border-t-2 border-l-2 border-[var(--color-bone)] group-hover:border-[var(--color-blood)] transition-colors" />
                  <span aria-hidden className="absolute -top-1.5 -right-1.5 w-3 h-3 border-t-2 border-r-2 border-[var(--color-bone)] group-hover:border-[var(--color-blood)] transition-colors" />
                  <span aria-hidden className="absolute -bottom-1.5 -left-1.5 w-3 h-3 border-b-2 border-l-2 border-[var(--color-bone)] group-hover:border-[var(--color-blood)] transition-colors" />
                  <span aria-hidden className="absolute -bottom-1.5 -right-1.5 w-3 h-3 border-b-2 border-r-2 border-[var(--color-bone)] group-hover:border-[var(--color-blood)] transition-colors" />

                  {/* The button itself */}
                  <div
                    className="
                      relative bg-[var(--color-blood)] text-[var(--color-void)]
                      px-6 py-3.5 border-2 border-[var(--color-void)]
                      flex items-center gap-3
                      transition-all duration-300
                      group-hover:bg-[var(--color-bone)]
                      group-hover:-translate-y-0.5
                      shadow-[0_0_0_3px_rgba(255,43,28,0.18),0_25px_45px_-10px_rgba(255,43,28,0.55)]
                      group-hover:shadow-[0_0_0_3px_rgba(245,241,234,0.25),0_30px_55px_-10px_rgba(255,43,28,0.7)]
                    "
                  >
                    {/* Hazard stripe sliver on the left edge */}
                    <span aria-hidden className="absolute left-0 top-0 bottom-0 w-1 hazard opacity-80" />
                    <span className="font-display text-xl sm:text-2xl tracking-wider whitespace-nowrap leading-none pl-2">
                      Take Down Madison, SD
                    </span>
                    <span className="font-mono text-sm leading-none group-hover:translate-x-1 transition-transform">
                      ◢
                    </span>
                  </div>
                </div>

                {/* Coordinates caption below */}
                <div className="font-mono text-[9px] uppercase tracking-[0.25em] text-[var(--color-mute)]">
                  44.0061° N · 97.1139° W
                </div>
              </Link>
            </div>
